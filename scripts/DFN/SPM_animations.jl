...
experiment_path = datadir("stochastic_rescaled_1e4")
...
parameter_filenames = sort(filter(x->occursin(r"[0-9][0-9][0-9][0-9][0-9].csv", x), readdir(experiment_path)))
last_parameter_filename = parameter_filenames[end]

last_par_vec = filename_to_parameter_vector(last_parameter_filename)
...
domain_t = (0.:dt:domain_t_end)
domain_rn = (0.:0.005:1.0)
domain_rp = (0.:0.005:1.0)

Q_evals = [chains_[1](t, last_par_vec)[1] for t in domain_t]
c_s_n_evals = [chains_[2]([t, rn], last_par_vec)[1] for t in domain_t, rn in domain_rn]
c_s_p_evals = [chains_[3]([t, rp], last_par_vec)[1] for t in domain_t, rp in domain_rp]