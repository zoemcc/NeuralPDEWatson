...
begin
    ...
    adaloss = NeuralPDE.LossGradientsAdaptiveLoss(20; α=0.9f0)
    ...
    exp_name = "minimax_consistent_loss_gradients"
...
params_lossgrad = pretrained_pinn_params

pretrained_params_file = joinpath(data_dir, "minimax_consistent", "50000.csv")
pretrained_pinn_params = Array(CSV.read(pretrained_params_file, DataFrame; header=false))[:,1]
params_minimax = pretrained_pinn_params
...
phi_i_params_lossgrad = [params_lossgrad[indices_in_params_i] for indices_in_params_i in indices_in_params]
phi_i_params_minimax = [params_minimax[indices_in_params_i] for indices_in_params_i in indices_in_params]

Q_evals_lossgrad = [chains_[1]([dt], phi_i_params_lossgrad[1])[1] for dt in sols_t]
Q_evals_minimax = [chains_[1]([dt], phi_i_params_minimax[1])[1] for dt in sols_t]

c_s_n_evals_pretrained_pinn_lossgrad = [chains_[2]([dt, drn], phi_i_params_lossgrad[2])[1] for dt in dts, drn in drns]
c_s_p_evals_pretrained_pinn_lossgrad = [chains_[3]([dt, drp], phi_i_params_lossgrad[3])[1] for dt in dts, drp in drps]
c_s_n_evals_pretrained_pinn_minimax = [chains_[2]([dt, drn], phi_i_params_minimax[2])[1] for dt in dts, drn in drns]
c_s_p_evals_pretrained_pinn_minimax = [chains_[3]([dt, drp], phi_i_params_minimax[3])[1] for dt in dts, drp in drps]
...
anim = @animate for i in 1:length(dts)
    ...
    p1 = plot!(p1,drns,c_s_n_evals_pretrained_pinn_lossgrad[i,:];ylims=(0,1.3),ylabel="c_s_n",xlabel="r_n",legend=true, label="PINN Loss Gradients", linestyle=:dashdot)
    p1 = plot!(p1,drns,c_s_n_evals_pretrained_pinn_minimax[i,:];ylims=(0,1.3),ylabel="c_s_n",xlabel="r_n",legend=true, label="PINN Minimax", linestyle=:dot)
    ...
    p2 = plot!(p2,drps,c_s_p_evals_pretrained_pinn_lossgrad[i,:];ylims=(0,1.3),ylabel="c_s_p",xlabel="r_p",legend=true, label="PINN Loss Gradients", linestyle=:dashdot)
    p2 = plot!(p2,drps,c_s_p_evals_pretrained_pinn_minimax[i,:];ylims=(0,1.3),ylabel="c_s_p",xlabel="r_p",legend=true, label="PINN Minimax", linestyle=:dot)
    plot(p1,p2)
end
gif(anim, joinpath(exp_folder, "SPM_fulltrain_multiple.gif"),fps=30)