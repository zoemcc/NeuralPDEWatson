...
#begin
    ...
    #Drn(c_s_n_xav(t, leftrdomain)) ~ 0.0,
    #Drn(c_s_n_xav(t, 1.0)) ~ -0.14182855923368468,
    Drp(c_s_p_xav(t, leftrdomain)) ~ 0.0,
    Drp(c_s_p_xav(t, 1.0)) ~ 0.03237700710041634,
    ]

    t_domain = IntervalDomain(0.0, 0.16) # 0.15
    rn_domain = IntervalDomain(leftrdomain, 1.0)
    rp_domain = IntervalDomain(leftrdomain, 1.0)
    ...
    adaloss = NeuralPDE.MiniMaxAdaptiveLoss(20; α_pde=0.3, α_bc=1e3, pde_weights_start=1e-3, bc_weights_start=1e3)
    data_dir = "/home/zobot/.julia/dev/NeuralPDEWatson/data"
    exp_name = "minimax_consistent_largerdomain_onlyc_s_p_adam_3"
...
begin
    ...
    gif(anim, joinpath(exp_folder, "SPM_fulltrain_adam3.gif"),fps=30)
end
main_train()